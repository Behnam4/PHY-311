...
\begin{document}
  ...
  \begin{enumerate}
    ...
    force on a charge $q$ placed at its tip?
      ...
      \textcolor{hwColor}{
        The general prescription for force between two charges is $F=k\dfrac{q Q}{r^2}$.
        \\
        \\
        \includegraphics[height=8cm, width=8cm]{C.JPG}
        \\
        Let's use $\overrightarrow{F}=Q \overrightarrow{E}$
        \\
        \\
        For the triangle created by the cone we have:
        \\
        \\
        $
          \begin{cases}
            \theta_1=\dfrac{\pi}{\theta_0}
            \\
            \\
            tan(\theta)=\dfrac{r}{L}
            \\
            ...
            \\
            \\
            dq=\sigma \left(2 \scriptr tan(\theta_0) sec(\theta_0)\right) L dL
          \end{cases}
        $
        \\
        \\
        \\
        All the horizontal forces cancel each other out so we have $\overrightarrow{\scriptr}=l \hat{L}$ (Due to symmetry)
        \\
        \\
        $
          \overrightarrow{E}(\overrightarrow{r})=\dfrac{1}{4 \pi \epsilon_0} \bigints \dfrac{1}{\scriptr^2} \hat{\scriptr} dq
          =\dfrac{1}{4 \pi \epsilon_0} \bigints\limits_{0}^{L} \dfrac{\hat{z}}{L^2} \sigma \scriptr L dL
          \\
          \\
          \\
          =\dfrac{\sigma \scriptr}{4 \pi \epsilon_0} ln\left(\dfrac{L}{o}\right) \hat{z}
          \\
          \\
          \\
          \therefore ~~~~ \overrightarrow{E}(\overrightarrow{r})=\dfrac{\sigma tan(\theta_0) sec(\theta_0)}{2 \epsilon_0} ln\left(\dfrac{L}{0}\right) \hat{z}
        $
        \\
        \\
        What we found diverges to $infty$. 
        \\
        \\
        $
          \therefore ~~~ \overrightarrow{F}=q \overrightarrow{E}
          \\
          \\
          \\
          \therefore ~~~ \overrightarrow{F}=\dfrac{q ~ \sigma ~ tan(\theta_0) sec(\theta_0)}{2 \epsilon_0} ln\left(\dfrac{L}{0}\right) \hat{z}
        $
        \\
        \\
      }
