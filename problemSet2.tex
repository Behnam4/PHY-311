...
\begin{document}
  ...
  \begin{enumerate}
    ...
    charge therefore experiences no force.
      ...
      \textcolor{hwColor}{
        \\
        \includegraphics[height=4cm, width=7cm]{Seven.JPG}
        \\
        On page 100 of the textbook, we have the above figure (similar to this problem). We have a electric charge $+q$ located off-center.
        Let's recall what the uniqueness theorem states (second). It states that \emph{in a volume $V$ surrounded 
        by conductors and containing a specific charge density $\rho$, the electric field is uniquely determined 
        if the total charge on each counductor is given.} 
        \\
        \\
        $+q$ attracts minus charges over to the near side and repel plus charges to the far side. Now there is a net force of attraction 
        between the test charge and the minus charges. In case, there is a cavity with some charge, then the field in the cavity is not zero.
        But the very important point is that the cavity and whatever inside it are electrically isolated from the outside world by the surrounding 
        conductor. 
        \\
        \\
        $Q_{enc}=q+q_{induced} \Longrightarrow q_{induced}=-q$. This means that the conductor as a whole is electrically neutral
        therefore, there must be a charge $+q$ on its outer surface. Based on what we discussed here the test charge $q$ experiences
        a force exerting from the outer surface on it, hence the argument is not correct. 
        \\
        \\
      }
    ...
    electric potential inside the pipe.
      ...
      \textcolor{hwColor}{
        \\
        \includegraphics[height=6cm, width=8cm]{Nine.jpg}
        \\
        We are asked to find the electric potential inside the pipe. One page 136 of the textbook there is a similar problem.
        So we are following the steps provided in its solutions.
        \\
        \\
        $
        ...
        \\
        \\
        But since in this problem the Laplacian is independent of x we have:
        \\
        \\
        $
          \dfrac{\partial^2 V}{\partial y^2}+\dfrac{\partial^2 V}{\partial z^2}=0
        $
        \\
        \\
        ...
        $
          ...
          \\
          \\
          \dfrac{\partial^2 YZ}{\partial y^2}+\dfrac{\partial^2 YZ}{\partial z^2}=0 
          \Longrightarrow Z ~ \dfrac{\partial^2 Y}{\partial y^2}+Y ~ \dfrac{\partial^2 Z}{\partial z^2}=0
          \\
          \\
          \\
          \dfrac{Z}{ZY} ~ \dfrac{\partial^2 Y}{\partial y^2}+\dfrac{Y}{ZY} ~ \dfrac{\partial^2 Z}{\partial z^2}=0
          \\
          \\
          \\
          Y^{-1} ~ \dfrac{\partial^2 Y}{\partial y^2}+Z^{-1} ~ \dfrac{\partial^2 Z}{\partial z^2}=0
          \\
          \\
          \\
          \begin{cases}
            c_1=Y^{-1} ~ \dfrac{\partial^2 Y}{\partial y^2}
            \\
            \\
            c_2=Z^{-1} ~ \dfrac{\partial^2 Z}{\partial z^2}
            \\
            \\
            c_1+c_2=0
          \end{cases}
        ...
        $
          ...
          \\
          \\
          V(y,z)=\left[C ~ sin(ky)+D ~ cos(ky)\right] \times \left[A ~ e^{kz}+B ~ e^{-kz}\right]
        $
        \\
        \\
